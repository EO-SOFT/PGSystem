...
import gui.packaging.mode2.state.S041_NewPalletScan;
...
public final class PACKAGING_UI9000_ChoosePackType extends javax.swing.JDialog {
    ...
    private void mappingValsToContext() {
        ...
        Helper.context.getBaseContainerTmp().setUcsId(Integer.valueOf(ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 0).toString().toString()));
        Helper.context.getBaseContainerTmp().setHarnessPart(Helper.HARN_PART_PREFIX + (String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 1));
        Helper.context.getBaseContainerTmp().setSupplierPartNumber((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 2));
        Helper.context.getBaseContainerTmp().setHarnessIndex((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 3));
        Helper.context.getBaseContainerTmp().setPackType((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 4));
        Helper.context.getBaseContainerTmp().setQtyExpected(Integer.valueOf(ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 5).toString()));
        Helper.context.getBaseContainerTmp().setStdTime(Double.valueOf(ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 6).toString()));
        Helper.context.getBaseContainerTmp().setAssyWorkstation((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 7));
        Helper.context.getBaseContainerTmp().setSegment((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 8));
        Helper.context.getBaseContainerTmp().setWorkplace((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 9));
        Helper.context.getBaseContainerTmp().setUcsLifes(Integer.valueOf(ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 10).toString()));
        Helper.context.getBaseContainerTmp().setComment((String) ucs_jtable.getValueAt(ucs_jtable.getSelectedRow(), 11));
    ...
    public void loadConfigUcs() {
        ...
        query.setParameter("harnessPart", Helper.context.getBaseContainerTmp().getHarnessPart().substring(1)); //items[0] = harnessPart
        query.setParameter("supplierPartNumber", Helper.context.getBaseContainerTmp().getSupplierPartNumber()); //items[1] = supplierPartNumber
        query.setParameter("harnessIndex", Helper.context.getBaseContainerTmp().getHarnessIndex()); //items[2] = harnessIndex                        
        query.setParameter("packType", Helper.context.getBaseContainerTmp().getPackType()); //items[3] = packType
        query.setInteger("packSize", Helper.context.getBaseContainerTmp().getQtyExpected()); //items[4] = packSize
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, String.format("Failed to found UCS matching criteria %s AND %s ",
                    Helper.context.getBaseContainerTmp().getSupplierPartNumber(), Helper.context.getBaseContainerTmp().getHarnessIndex()), "Database Qery Error!", ERROR_MESSAGE);
        }
        ...
        if (palletId != -1) {
            ...
            //############# PASSE TO S041 STATE ###############
            Helper.log.info(String.format("Openning new container for first harness part [%s].", Helper.context.getBaseContainerTmp().getHarnessPart().substring(1)));
            ...
            S041_NewPalletScan state = new S041_NewPalletScan();
            Helper.context.setState(state);
    ...
    private void formWindowClosing(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowClosing
        clearContextSessionVals();
        // Change go back to state HarnessPartScan            
        Helper.context.setState(new S020_HarnessPartScan());
        Helper.Packaging_Gui_Mode1.setRequestedPallet_txt("");
        ...
        if (evt.getKeyCode() == KeyEvent.VK_ENTER || evt.getKeyCode() == KeyEvent.VK_SPACE) {
            ...
            if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                query.setParameter("harnessPart", Helper.context.getBaseContainerTmp().getHarnessPart().substring(1))
                        ...
                        .setParameter("packType", Helper.context.getBaseContainerTmp().getPackType());
                Helper.sess.getTransaction().commit();
                result = query.list();
                ...
                if (result.isEmpty()) {
                    if (Integer.valueOf(Helper.PROP.getProperty("UNIQUE_PALLET_PER_PACK_TYPE")) == 1) {
                        ...
                        query.setParameter("packType", Helper.context.getBaseContainerTmp().getPackType())
                                .setParameter("containerState", Helper.PALLET_OPEN)
                        ...
                        if (result.isEmpty()) {
                            ...
                            JOptionPane.showMessageDialog(null, String.format(Helper.ERR0025_PACKTYPE_ALREADY_OPEN_IN_THE_SAME_WORKSTATION,
                                    Helper.context.getBaseContainerTmp().getPackType(),
                                    Helper.HOSTNAME,
                                    bc.getHarnessPart(),
                                    bc.getPalletNumber(),
                                    Helper.context.getBaseContainerTmp().getPackType()),
                    ...
                    JOptionPane.showMessageDialog(null, String.format(Helper.ERR0024_PALLET_ALREADY_OPEN, Helper.context.getBaseContainerTmp().getHarnessPart().substring(1)), "Palette déjà ouverte !", JOptionPane.ERROR_MESSAGE);
                    Helper.log.info(String.format(Helper.ERR0024_PALLET_ALREADY_OPEN, Helper.context.getBaseContainerTmp().getHarnessPart().substring(1)));
